...
class ArduinoBridge:
    ...
    def __init__(self, port: str = "/dev/ttyUSB0", baudrate: int = 9600, timeout: float = 5.0):
        """Inicializa bridge."""
        self.port = port
    ...
    def notify_embryo_injected(self, timestamp: str, volume_ul: float) -> bool:
        ...
        try:
            ...
            if response == "OK":
                self.logger.info("✓ Arduino notificado: embrião injetado")
                return True
            else:
    ...
    def get_incubator_status(self) -> dict | None:
        """
    ...
    def get_incubation_day(self) -> int | None:
    ...
    def get_full_logs(self) -> dict | None:
