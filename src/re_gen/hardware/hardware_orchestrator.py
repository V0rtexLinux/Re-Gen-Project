...
from re_gen.hardware.hardware_devices import build_hardware_devices, get_default_device_order
...
class OrchestrationJob:
    ...
    injection_timestamp: str | None = None
...
class HardwareOrchestrator:
    ...
    def __init__(self, arduino_port: str = "/dev/ttyUSB0", arduino_baudrate: int = 9600):
        """Inicializa orquestrador."""
        self.logger = logging.getLogger("HardwareOrchestrator")

        # Estado global
        self.state = HardwareState.IDLE
        self.current_job: OrchestrationJob | None = None
    ...
    def get_device(self, name: str) -> DeviceBase | None:
    ...
    def execute_complete_workflow(self, species_name: str) -> dict | None:
        """Executa workflow completo: DNA → Injeção → Incubação."""

        job_id = f"orch_{species_name.replace(' ', '_')}_{int(time.time())}"
        self.current_job = OrchestrationJob(job_id=job_id, species_name=species_name)
    ...
    def _execute_synthesis(self, species_name: str) -> float | None:
        ...
        try:
            ...
            dntp_config = DNTPoolConfig(
                dATP_mM=100.0, dTTP_mM=100.0, dGTP_mM=100.0, dCTP_mM=100.0, buffer_pH=7.5, Mg2_mM=5.0, temp_celsius=72.0
            )
    ...
    def _execute_injection(self, species_name: str, timestamp: str, dna_volume: float) -> bool:
        """Executa injeção de DNA."""
        self.logger.info(f"Executando injeção de DNA ({dna_volume}µL)...")
        self.current_job.log_event("INJECT_START", "Iniciando injeção")
        ...
        try:
            ...
            success = injector.inject_dna(f">{species_name}_injected|{timestamp}", target, dna_concentration_ng_ul=50.0)
    ...
    def _notify_incubator(self, timestamp: str, dna_volume: float) -> bool:
        ...
        try:
            ...
            success = incubator.notify_embryo_injected(timestamp=timestamp, volume_ul=dna_volume)
