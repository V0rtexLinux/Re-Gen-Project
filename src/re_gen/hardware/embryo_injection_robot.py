...
def _require_gpio() -> None:
    if not _HAS_GPIO:
        raise ImportError("RPi.GPIO is required for real hardware operation. Install with: pip install RPi.GPIO")
...
class ServoMotor:
    ...
    def angle_to_pulse_width(self, angle: float) -> float:
        ...
        pulse_ms = ServoConfig.MIN_PULSE_WIDTH_MS + (angle / 180.0) * (
            ServoConfig.MAX_PULSE_WIDTH_MS - ServoConfig.MIN_PULSE_WIDTH_MS
        )
...
class XYZStage:
    ...
    def move_to(self, x_mm: float, y_mm: float, z_mm: float, speed: float = 0.5):
...
class SyringeController:
    ...
    def inject_volume(self, volume_nl: float):
        ...
        if volume_ul > self.current_volume_ul:
            self.logger.error("Seringa não tem volume suficiente")
            return False
...
class EmbryoInjectionRobot:
    ...
    def find_embryo_nucleus(self, image) -> InjectionTarget | None:
    ...
    def inject_dna(self, dna_sequence: str, target: InjectionTarget, dna_concentration_ng_ul: float = 50.0) -> bool:
