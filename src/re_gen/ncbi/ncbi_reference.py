...
class DeepReferenceSearchEngine:
    ...
    def __init__(self, email: str, api_key: str | None = None, output_dir: str = "./deep_search_results"):
    ...
    def search_comprehensive(self, target_lineage: str = "dinosauria") -> list[ReferenceSequenceDeep]:
        ...
        for group_name, group_info in sorted(self.REFERENCE_GROUPS.items(), key=lambda x: x[1]["priority"]):
                ...
                for species in group_info["taxa"]:
                    try:
                        ...
                        if ref:
                            ...
                            ref.phylogenetic_distance = self._estimate_phylogenetic_distance(species, group_name)
                            ...
                            if key not in all_results or ref.quality_score > all_results[key].quality_score:
                                all_results[key] = ref
                                self.logger.info(
                                    f"  ✓ {species}/{gene}: {len(ref.sequence):,}bp (Q={ref.quality_score:.0f})"
                                )
    ...
    def _fetch_single_sequence(self, species: str, gene: str) -> ReferenceSequenceDeep | None:
...
class NCBIQueryError(Exception):
    """Erro durante busca no NCBI."""


@dataclass
class ReferencePanel:
    """Painel de referências para reconstrução ancestral."""
...
def buscar_painel_referencia_por_paleontologia(
    dinossauro: object,
    gene: str,
    email: str,
    api_key: str | None = None,
) -> list[ReferencePanel]:
    """Busca painel de referências NCBI usando descendentes vivos mapeados filogeneticamente.

    Args:
        dinossauro: Objeto Dinossauro com descendentes_vivos.
        gene: Nome do gene a buscar (ex: 'cytochrome b').
        email: E-mail registrado no NCBI (obrigatório).
        api_key: API key opcional do NCBI.

    Returns:
        Lista de ReferencePanel com sequências dos descendentes vivos.

    Raises:
        NCBIQueryError: Se a busca falhar.
    """
    from re_gen.data.paleontology import Dinossauro

    if not isinstance(dinossauro, Dinossauro):
        raise NCBIQueryError("Objeto dinossauro inválido")

    descendentes = getattr(dinossauro, "descendentes_vivos", [])
    if not descendentes:
        raise NCBIQueryError(f"Nenhum descendente vivo mapeado para {dinossauro.nome_cientifico}")

    engine = DeepReferenceSearchEngine(email=email, api_key=api_key)
    panels: list[ReferencePanel] = []

    for descendente in descendentes:
        try:
            refs = engine._fetch_single_sequence(descendente, gene)
            if refs is not None:
                panels.append(
                    ReferencePanel(
                        species=refs.species,
                        accession=refs.accession,
                        gene_name=refs.gene_name,
                        sequence=refs.sequence,
                        length=refs.length,
                        fonte_genealogica=f"{dinossauro.nome_cientifico} → {descendente}",
                    )
                )
            time.sleep(0.4)
        except Exception as exc:
            logger.debug("Erro ao buscar %s/%s: %s", descendente, gene, exc)

    if not panels:
        raise NCBIQueryError(
            f"Nenhuma referência encontrada para gene '{gene}' entre descendentes de {dinossauro.nome_cientifico}"
        )
