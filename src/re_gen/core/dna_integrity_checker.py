...
class DNAIntegrityChecker:
    ...
    def full_integrity_check(self, sequence: str, reference_hashes: dict | None = None) -> CompleteIntegrityReport:
        """
    ...
    def _check_cryptographic(self, sequence: str, ref_hashes: dict | None = None) -> None:
    ...
    def _check_structural(self, sequence: str) -> None:
        ...
        self.results.append(
            IntegrityCheckResult(
                ...
                details=f"Bases válidas: {total_valid}/{total} ({validity_ratio * 100:.1f}%), N's: {n_count}",
            )
        )
    ...
    def _check_biological(self, sequence: str) -> None:
        ...
        self.results.append(
            IntegrityCheckResult(
                ...
                score=orf_score,
                status="PASS",
                details=f"Codons iniciais (ATG/GTG/TTG): {starts}, Codons terminais (TAA/TAG/TGA): {stops}",
            )
        )
    ...
    def _check_phylogenetic(self, sequence: str) -> None:
        ...
        self.results.append(
            IntegrityCheckResult(
                ...
                score=score,
                status="PASS",
                details=f"Comprimento: {len(seq_upper):,}bp. Conservação estimada vs referências: esperado 70-90%",
            )
        )
    ...
    def generate_report(self, report: CompleteIntegrityReport, output_path: str | None = None) -> str:
