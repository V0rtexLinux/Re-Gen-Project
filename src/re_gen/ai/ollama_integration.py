...
class ClienteOllama:
    ...
    def __init__(self, config: ConfiguracaoOllama | None = None):
    ...
    @overload
    def gerar_texto(self, prompt: str, modelo: str | None = ..., *, streaming: Literal[False] = ...) -> str: ...

    @overload
    def gerar_texto(
        self, prompt: str, modelo: str | None = ..., *, streaming: Literal[True]
    ...
    def gerar_texto(
        ...
        if not self.validar_conexao():
            raise RuntimeError(f"Ollama não está acessível em {self.config.endpoint}. Inicie com: `ollama serve`")
    ...
    def _gerar_simples(self, url: str, payload: dict) -> str:
        ...
        for tentativa in range(self.config.max_retries):
            try:
                ...
                if resp.status_code == 200:
                    resultado: dict[str, str] = resp.json()
                ...
                if tentativa < self.config.max_retries - 1:
                    ...
                    raise RuntimeError(f"Falha na requisição Ollama após retries: {e}") from e
        ...
        for tentativa in range(self.config.max_retries):
            try:
                ...
                if tentativa < self.config.max_retries - 1:
                    ...
                    raise RuntimeError(f"Falha no streaming Ollama: {e}") from e

    def embeddings(self, texto: str, modelo: str | None = None) -> list[float]:
        """
        ...
        try:
            ...
            if resp.status_code == 200:
                dados: dict[str, list[float]] = resp.json()
