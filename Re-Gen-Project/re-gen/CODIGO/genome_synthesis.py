...
class RealSequenceBuilder:
    ...
    def build_from_reference_panel(
        ...
        longest_idx = max(range(len(sequences_for_alignment)),
                         key=lambda i: len(sequences_for_alignment[i].seq))
        ...
        for pos in range(alignment_length):
            ...
            for (anchor_aln, ref_aln), metadata in zip(alignments, metadata_list):
                if pos < len(ref_aln):
                    base = ref_aln[pos]
                    if base in "ATGC":
            ...
            if not votes:
                ...
                positions_data.append(ReconstructedGenomePosition(
                    position=pos,
                    base="N",
                    confidence=0.0,
        ...
        self.logger.info(f"✓ Reconstrução concluída: {len(consensus_seq):,}bp, "
                        f"confiança média={mean_conf:.1f}%")
    ...
    def _save_reconstruction(self, result: RealDinosaurSequence) -> None:
        ...
        with open(metadata_path, "w") as f:
            json.dump({
                ...
                "low_confidence_regions": [
                    {"start": s, "end": e} for s, e in result.low_confidence_regions
                ],
        ...
        with open(positions_path, "w") as f:
            ...
            for p in result.positions_metadata:
                f.write(f"{p.position},{p.base},{p.confidence:.1f},"
