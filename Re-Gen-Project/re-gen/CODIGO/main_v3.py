...
from genome_synthesis import GenomeSynthesizer, GenomeSynthesisJob, create_test_genome_job, ChunkStatus
from ai_tools import get_tool_registry, ToolType
from genome_validator import GenomeValidator, create_validator
...
from ollama_integration import obter_cliente_ollama, ConfiguracaoOllama

# Importa novo sistema de dinossauros
try:
    from dinosaur_database import database_stats, get_dinosaurs_by_popularity, get_dinosaur_by_name
...
class OrchestrationContext:
    ...
    def __init__(self, job: GenomeSynthesisJob, ncbi_email: str, ncbi_api_key: Optional[str] = None):
        self.job = job
...
"""
    ...
    def _generate_default_synthesis_strategy(self) -> dict:
        ...
        tool_calls.append({
            ...
            "arguments": {
                "query": f"{self.context.job.target_species} genome complete sequence",
                "max_results": 3
            }
        })
        ...
        tool_calls.append({
            "name": "design_grna",
            "arguments": {
                "sequence": "ATGCATGCATGCATGC" * (chunk_size // 16),
                "target_region": "0-1000"
            }
        })
...
def main() -> int:
    ...
    if args.list_dinosaurs:
        ...
        print(f"Períodos: Triássico ({stats['periodos']['triassico']}), "
    ...
    if args.use_ai_orchestration:
        ...
        print(f"  Target: {job.target_genome_size_bp/1e9:.1f} Gb em {job.expected_chunks:,} chunks de {job.chunk_size_bp/1000:.0f}kb")
