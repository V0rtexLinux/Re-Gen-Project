...
from scanner_input import load_scanner_output, summarize_reads, ScannerInputError
from ncbi_reference import (
    buscar_painel_referencia_por_paleontologia,
    NCBIQueryError,
)
from reconstruct import reconstruct_ancestral_sequence, low_confidence_regions
from gene_edit_package import build_edit_package, export_edit_package_csv
from paleontology import obter_sistema_referencia, Dinossauro
...
def selecionar_dinossauro_automaticamente(
    hardware: str,
    preferencia_dieta: Optional[str] = None,
    preferencia_tamanho_min: Optional[float] = None,
    preferencia_tamanho_max: Optional[float] = None,
...
def main() -> int:
    ...
    with open(consensus_fasta, "w") as f:
        f.write(f">Re-Dino_{dinossauro.nome_cientifico.replace(' ', '_')}|confianca={reconstruction.mean_confidence:.3f}\n")
    ...
    print(f"     → Envie para fornecedor de síntese (Twist Bioscience, IDT)")
    print(f"\n  2. Pacote de edição genética: {edit_csv.name}")
    print(f"     → Importe em ferramenta de CRISPR (Benchling, CHOPCHOP)")
    
    if (out_dir / "laudo_ia.txt").exists():
        print(f"\n  3. Laudo técnico: laudo_ia.txt")
        print(f"     → Documentação para laboratório")
