...
class ValidationIssue:
    ...
    position: Optional[int] = None          # Posição no genoma
    description: str = ""
    suggested_fix: Optional[str] = None
    affected_region: Optional[str] = None   # Sequência afetada
    ...
    def __str__(self) -> str:
        ...
        if self.issues:
            ...
            for issue in self.issues:
                ...
                lines.append(
                    f"  {severity_emoji.get(issue.severity, '')} {issue.issue_type}: {issue.description}"
                )
