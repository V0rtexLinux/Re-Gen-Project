...
class ReedSolomonEncoder:
    ...
    @staticmethod
    def decode_block(encoded: bytes, n_parity: int = 8) -> Optional[bytes]:
...
class DNAStorageMediaWriter:
    ...
    def prepare_for_dvd(self, media_type: str = "DVD-R") -> Dict:
        ...
        return {
            ...
            "total_size_bytes": sum(
                f.stat().st_size
                for f in (self.output_dir / self.dinosaur_name).rglob("*")
                if f.is_file()
            ),
            "media_type": media_type,
    ...
    def _write_2bit(self, path: Path) -> None:
        ...
        self.logger.info(f"2-bit escrito: {path.name} ({path.stat().st_size} bytes, "
